...
export const updateNodeColor = (
  ...
  try {
    ...
    if (color === undefined || color.hex === undefined || color.hex === '' || typeof color.hex !== 'string' || !color.hex.startsWith('#')) {
      console.error('Invalid color format:', color);
      return;
    }

    console.log(`DEBUG: updateNodeColor called with nodeId: ${nodeId}, hex: ${color.hex}, isPreview: ${String(isPreview)}, directNode provided: ${String(directNode !== undefined)}`);
    ...
    if (directNode !== undefined) {
      ...
      console.log(`DEBUG: Current selection count: ${currentSelection.length}, selection IDs: ${currentSelection.map(n => n.id).join(', ')}`);
      ...
      if (node === null) {
        console.log('DEBUG: Node not found in selection, searching entire page...');
        const nodes = figma.currentPage.findAll(n => n.id === nodeId);
        console.log(`DEBUG: Found ${nodes.length} nodes matching ID: ${nodeId}`);
        ...
        try {
          ...
          if (typeof figmaAny.getNodeById === 'function') {
            ...
            if (foundNode !== null && foundNode !== undefined) {
              ...
              console.log('DEBUG: Node found via getNodeById, type:', node.type);
    ...
    if ('fills' in node) {
      ...
      console.log(`DEBUG: Converting hex ${color.hex} to RGB: r=${r}, g=${g}, b=${b}`);
      
      // Get current fills
      const fills = JSON.parse(JSON.stringify(node.fills)) as FigmaPaint[];
      console.log(`DEBUG: Current fills count: ${fills.length}, types: ${fills.map(f => f.type).join(', ')}`);
      ...
      if (solidFillIndex !== -1) {
        ...
        console.log(`DEBUG: Current color at index ${solidFillIndex}:`, newFills[solidFillIndex].color);
        ...
        console.log(`DEBUG: New color at index ${solidFillIndex}:`, newFills[solidFillIndex].color);
